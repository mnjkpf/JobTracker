...
@Component
public class NoFluffJobsParser implements JobBoardParser {

    private static final Logger LOGGER = LoggerFactory.getLogger(NoFluffJobsParser.class);
    ...
    public Optional<ParsedJobPosting> parse(String html, String url) {
        ...
        // Витягуємо обов'язкові поля
        String position = tryExtract(doc, SELECTOR_POSITION);
        String company = tryExtract(doc, SELECTOR_COMPANY);

        // Без обов'язкових → empty → orchestrator піде на LLM fallback
        if (position == null || company == null) {
            LOGGER.warn("Missing required fields in NoFluffJobs HTML: {}", url);
            return Optional.empty();
        }

        // Опціональні поля — null якщо не знайдено
        String location = tryExtract(doc, SELECTOR_LOCATION);
        String description = tryExtract(doc, SELECTOR_DESCRIPTION);

        // Salary і enum-поля можуть мати свою логіку парсингу
    ...
    /** Безпечне витягання — null якщо селектор нічого не знайшов. */
    ...
    private SalaryRange parseSalary(Document doc) {
        // Приклад: "8 000 - 12 000 PLN"
        String raw = tryExtract(doc, "span.salary-range");
        if (raw == null) return null;
        // Regex для "8 000 - 12 000 PLN"
    ...
    /** Простий внутрішній value-record. Можна замінити на іменований static-клас. */
